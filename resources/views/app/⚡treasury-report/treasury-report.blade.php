<div>
    <x-header :title="__('Monthly Treasury Reports')" :subtitle="__('Generate, manage, and publish financial transparency reports')" separator>
        <x-slot:actions>
            <x-button icon="o-document-plus" :label="__('Generate Report')" class="bg-gradient-to-r from-primary to-secondary text-white border-none shadow-lg shadow-primary/30 hover:scale-105 transition-transform" wire:click="openModal()" />
        </x-slot:actions>
    </x-header>
        ...
        @forelse($reports as $r)
            <div class="bg-base-100 rounded-2xl border border-base-content/5 shadow-sm overflow-hidden flex flex-col group" wire:key="report-{{ $r->id }}">
                <div class="bg-gradient-to-br from-slate-900 to-indigo-950 px-5 py-4 text-white relative overflow-hidden">
                    ...
                    <div class="flex items-center justify-between relative z-10">
                        <div>
                            ...
                            <p class="text-xs text-white/50 font-semibold mt-0.5">{{ __('Report') }} #{{ str_pad($r->id, 4, '0', STR_PAD_LEFT) }}</p>
                        </div>
                        @if($r->is_published)
                            <div class="w-8 h-8 rounded-full bg-emerald-500/20 text-emerald-400 flex items-center justify-center ring-1 ring-emerald-500/50" :title="__('Published to Members')">
                                <x-icon name="o-eye" class="w-4 h-4" />
                            </div>
                        @else
                            <div class="w-8 h-8 rounded-full bg-white/10 text-white/50 flex items-center justify-center ring-1 ring-white/20" :title="__('Draft (Hidden)')">
                ...
                <div class="p-5 flex-grow space-y-4">
                    ...
                    <div class="pt-3 border-t border-base-content/10">
                        <div class="flex justify-between items-end">
                            <span class="text-xs font-bold uppercase tracking-wider text-base-content/40">{{ __('Closing') }}</span>
                ...
                <div class="p-3 bg-base-200/50 border-t border-base-content/5 flex items-center gap-1">
                    <x-button class="btn-ghost btn-sm flex-1" :icon="$r->is_published ? 'o-eye-slash' : 'o-eye'" :class="$r->is_published ? 'text-warning' : 'text-success'" wire:click="togglePublish({{ $r->id }})" :tooltip="$r->is_published ? __('Unpublish') : __('Publish')" />
                    <x-button icon="o-information-circle" class="btn-ghost btn-sm text-info" wire:click="viewDetails({{ $r->id }})" :tooltip="__('View Breakdown')" />
                    <x-button icon="o-pencil" class="btn-ghost btn-sm" wire:click="openModal({{ $r->id }})" :tooltip="__('Edit')" />
                    <x-button icon="o-printer" class="btn-ghost btn-sm" onclick="window.print()" :tooltip="__('Print')" />
                    <x-button icon="o-trash" class="btn-ghost btn-sm text-error" wire:click="delete({{ $r->id }})" wire:confirm="{{ __('Delete this report?') }}" :tooltip="__('Delete')" />
                </div>
            </div>
        @empty
            <div class="col-span-full text-center py-20 bg-base-200/50 rounded-2xl border border-dashed border-base-content/10">
                ...
                <h3 class="text-lg font-bold text-base-content/70 mb-1">{{ __('No reports generated yet') }}</h3>
                <p class="text-base-content/50 text-sm mb-4">{{ __('Generate your first monthly treasury report.') }}</p>
                <x-button icon="o-document-plus" :label="__('Generate Report')" class="btn-primary" wire:click="openModal()" />
            </div>
        @endforelse
    </div>
    ...
    {{-- Generate/Edit Modal --}}
    <x-modal wire:model="modal" :title="$editingId ? __('Edit Report') : __('Generate Monthly Report')" class="backdrop-blur-sm" box-class="w-full max-w-2xl">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            <x-input wire:model.live="year" :label="__('Year *')" type="number" min="2020" class="input-bordered rounded-xl" />
            <div>
                <label class="label"><span class="label-text font-semibold">{{ __('Month *') }}</span></label>
        ...
        <div class="bg-base-200/50 rounded-2xl p-5 border border-base-content/10 mb-6 relative overflow-hidden">
            @if(!$editingId)
                <div class="absolute top-2 right-2 text-[10px] uppercase font-bold tracking-widest text-primary bg-primary/10 px-2 py-0.5 rounded-full">{{ __('Auto-Calculated') }}</div>
            @endif
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    <p class="text-xs font-semibold text-base-content/50 uppercase">{{ __('Total Income') }}</p>
                    <p class="text-xl font-black text-emerald-600">৳{{ number_format($calc_income, 2) }}</p>
                </div>
                <div>
                    <p class="text-xs font-semibold text-base-content/50 uppercase">{{ __('Total Expenses') }}</p>
                    <p class="text-xl font-black text-rose-600">৳{{ number_format($calc_expense + $calc_fees, 2) }}</p>
                    @if($calc_fees > 0)
                        <p class="text-[10px] text-rose-600/70">{{ __('(incl. ৳:amount fees)', ['amount' => $calc_fees]) }}</p>
                    @endif
                </div>
            </div>
            <div class="pt-4 border-t border-base-content/10 grid grid-cols-1 sm:grid-cols-2 gap-4 items-end">
                <x-input wire:model.live="opening_balance" :label="__('Opening Balance (৳) *')" type="number" step="0.01" class="input-bordered rounded-xl bg-base-100" />
                <div class="pb-2">
                    <p class="text-xs font-semibold text-base-content/50 uppercase mb-1">{{ __('Projected Closing Balance') }}</p>
                    <p class="text-2xl font-black {{ $this->closingBalance >= 0 ? 'text-primary' : 'text-rose-600' }}">৳{{ number_format($this->closingBalance, 2) }}</p>
                </div>
            </div>
        </div>

        <x-textarea wire:model="notes" :label="__('Treasurer Notes / Summary')" :placeholder="__('Optional comments for the community...')" rows="3" class="textarea-bordered rounded-xl" />

        <x-slot:actions>
            <x-button :label="__('Cancel')" wire:click="$set('modal', false)" class="btn-ghost" />
            <x-button :label="$editingId ? __('Save Changes') : __('Generate & Save')" icon="o-check" wire:click="save" class="bg-gradient-to-r from-primary to-secondary text-white border-none font-bold" spinner="save" />
        </x-slot:actions>
    </x-modal>

    {{-- Details Breakdown Modal --}}
    <x-modal wire:model="detailsModal" :title="__('Report Breakdown') . ' - ' . $viewingReport?->month_name" class="backdrop-blur-sm" box-class="w-full max-w-3xl">
        @if($viewingReport)
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="bg-emerald-50 dark:bg-emerald-900/10 rounded-2xl p-5 border border-emerald-100 dark:border-emerald-900/30">
                    <h4 class="font-bold text-emerald-800 dark:text-emerald-400 mb-4 flex items-center gap-2">
                        <x-icon name="o-arrow-down-tray" class="w-5 h-5" /> {{ __('Income Sources') }}
                    </h4>
                    <div class="space-y-3">
                        ...
                        @empty
                            <p class="text-sm text-emerald-600/50">{{ __('No specific income data found.') }}</p>
                        @endforelse
                    </div>
                ...
                <div class="bg-rose-50 dark:bg-rose-900/10 rounded-2xl p-5 border border-rose-100 dark:border-rose-900/30">
                    <h4 class="font-bold text-rose-800 dark:text-rose-400 mb-4 flex items-center gap-2">
                        <x-icon name="o-receipt-percent" class="w-5 h-5" /> {{ __('Expenditures') }}
                    </h4>
                    <div class="space-y-3">
                        ...
                        @empty
                            <p class="text-sm text-rose-600/50">{{ __('No specific expense data found.') }}</p>
                        @endforelse
                        @if($viewingReport->total_transfer_fees > 0)
                            <div class="flex justify-between items-center text-sm border-b border-rose-200 dark:border-rose-800/50 pb-2 last:border-0">
                                <div class="flex items-center gap-2">
                                    <div class="w-2.5 h-2.5 rounded-full bg-amber-500"></div>
                                    <span class="font-medium">{{ __('Transfer Fees') }}</span>
                                </div>
            ...
            <div class="mt-6 flex justify-between items-center p-4 bg-base-200/50 rounded-xl">
                <div>
                    <span class="text-xs uppercase font-bold text-base-content/50">{{ __('Opening Balance') }}</span>
                    <p class="text-lg font-black">৳{{ number_format($viewingReport->opening_balance) }}</p>
                </div>
                <div class="text-right">
                    <span class="text-xs uppercase font-bold text-base-content/50">{{ __('Closing Balance') }}</span>
        ...
        <x-slot:actions>
            <x-button :label="__('Close')" wire:click="$set('detailsModal', false)" class="btn-primary" />
        </x-slot:actions>
    </x-modal>
