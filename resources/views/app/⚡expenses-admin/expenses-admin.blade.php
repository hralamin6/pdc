<div>
    <x-header title="Expenses Management" subtitle="Track and manage community expenditures" separator>
        <x-slot:actions>
            <x-button icon="o-plus" label="Record Expense" class="bg-gradient-to-r from-primary to-secondary text-white border-none shadow-lg shadow-primary/30 hover:scale-105 transition-transform" wire:click="openModal()" />
        </x-slot:actions>
    </x-header>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div class="lg:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-4">
            <x-input wire:model.live.debounce.300ms="search" placeholder="Search by title..." icon="o-magnifying-glass" class="input-bordered rounded-xl" clearable />
        ...
        <div class="bg-gradient-to-br from-rose-500 to-rose-600 rounded-2xl p-5 text-white shadow-lg shadow-rose-500/30 flex flex-col justify-center">
            <p class="text-xs font-bold uppercase tracking-widest text-rose-100 mb-1">Total Found</p>
                                ...
                                <tr class="hover:bg-base-200/50 transition-colors" wire:key="expense-{{ $ex->id }}">
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-xs text-base-content/50">
                                                    {{ $ex->category->name ?? 'Uncategorized' }}
                                                    @if($ex->linkable)
                                                        &bull; Linked: {{ class_basename($ex->linkable_type) }}
                                                    @endif
                                                </p>
                                            </div>
                                        </div>
                                    ...
                                    <td>
                                        <div class="flex gap-1">
                                            @if($ex->receipt_url)
                                                <a href="{{ $ex->receipt_url }}" target="_blank" class="btn btn-ghost btn-xs text-primary" title="View Receipt">
                                                    <x-icon name="o-paper-clip" class="w-4 h-4" />
                                                </a>
                                            @endif
                                            <x-button icon="o-pencil" class="btn-ghost btn-xs" wire:click="openModal({{ $ex->id }})" tooltip="Edit" />
                                            <x-button icon="o-trash" class="btn-ghost btn-xs text-error" wire:click="delete({{ $ex->id }})" wire:confirm="Delete this expense?" tooltip="Delete" />
                                        </div>
                                    </td>
                                </tr>
                            @empty
        ...
        <div class="xl:col-span-1 space-y-6">
            @if(count($chartData) > 0)
                <div class="bg-base-100 rounded-2xl border border-base-content/5 p-6 shadow-sm">
                    <h3 class="font-bold text-lg mb-6">Category Breakdown</h3>
                    ...
                    <div class="relative w-full aspect-square max-h-[300px] mx-auto" 
                        ...
                        <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                            <span class="text-xs text-base-content/50 uppercase font-bold tracking-widest">Total</span>
            ...
            @else
                <div class="bg-base-100 rounded-2xl border border-base-content/5 p-6 shadow-sm text-center">
                    ...
                    <p class="text-base-content/50 text-sm">No category data for selected month.</p>
                </div>
            @endif
        </div>
    </div>
    ...
    {{-- Expense Modal --}}
    <x-modal wire:model="expenseModal" title="{{ $editingId ? 'Edit Expense' : 'Record Expense' }}" class="backdrop-blur-sm" box-class="w-full max-w-3xl">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <x-input wire:model="title" label="Title/Description *" placeholder="e.g. Monthly Iftar Food" class="input-bordered rounded-xl md:col-span-2" />
            ...
            <div class="space-y-4">
                ...
                <x-input wire:model="amount" label="Amount (৳) *" type="number" step="0.01" min="1" placeholder="0.00" class="input-bordered rounded-xl" />
                
                <x-input wire:model="expense_date" label="Date *" type="date" class="input-bordered rounded-xl" />
            </div>
            ...
            <div class="space-y-4">
                ...
                @if(in_array($payment_method, ['bkash', 'nagad', 'bank_transfer']))
                    <div>
                        <label class="label"><span class="label-text font-semibold">Source Bank Account</span></label>
                        <select wire:model="bank_account_id" class="select select-bordered w-full rounded-xl">
                            <option value="0">Not specified / External</option>
                            @foreach($bankAccounts as $acc)
                                <option value="{{ $acc->id }}">{{ $acc->name }} ({{ ucfirst($acc->type) }})</option>
                            @endforeach
                        </select>
                    </div>
                    <x-input wire:model="transaction_id" label="Transaction ID / Ref" placeholder="TxID" class="input-bordered rounded-xl" />
                @endif
            </div>

            {{-- Linking --}}
            <div class="md:col-span-2 bg-base-200/50 rounded-xl p-4 border border-base-content/5">
                <p class="font-bold text-sm mb-3 text-base-content/70"><x-icon name="o-link" class="w-4 h-4 inline" /> Link to Project (Optional)</p>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <select wire:model.live="linkable_type" class="select select-sm select-bordered w-full rounded-lg">
                            <option value="">No link</option>
                            <option value="App\Models\DonationCampaign">Campaign</option>
                            <option value="App\Models\Halaqah">Halaqah Session</option>
                        </select>
                    </div>
                    <div>
            ...
            <div class="md:col-span-2">
                <x-textarea wire:model="notes" label="Additional Notes" placeholder="Any extra details..." rows="2" class="textarea-bordered rounded-xl" />
            </div>

            <div class="md:col-span-2">
                <x-file wire:model="receipt" label="Receipt Image (optional)" accept="image/*" class="file-input-bordered w-full" hint="Max 5MB. Clear image of receipt/memo." />
                @if($receipt)
                    <div class="mt-2 text-emerald-600 text-sm font-semibold">New image selected for upload.</div>
                @endif
            </div>
        </div>

        <x-slot:actions>
            <x-button label="Cancel" wire:click="$set('expenseModal', false)" class="btn-ghost" />
            <x-button label="{{ $editingId ? 'Update' : 'Record Expense' }}" icon="{{ $editingId ? 'o-check' : 'o-receipt-percent' }}" wire:click="save" class="bg-gradient-to-r from-primary to-secondary text-white border-none font-bold" spinner="save" />
        </x-slot:actions>
    </x-modal>
