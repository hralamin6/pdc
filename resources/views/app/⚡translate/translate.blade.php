<div class="space-y-6" x-data="{
            ...
            <div class="bg-base-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                ...
                <div class="flex items-end justify-between">
                    ...
                    <div class="flex gap-1">
                        <button wire:click="exportLanguage('{{ $code }}')" class="btn btn-ghost btn-xs" title="{{ __('Export') }}">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
                            </svg>
                        </button>
                        @if($code !== 'en')
                            <button wire:click="deleteLanguage('{{ $code }}')" wire:confirm="{{ __('Delete :language?', ['language' => $stat['language']]) }}" class="btn btn-ghost btn-xs text-error" title="{{ __('Delete') }}">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
    ...
    <div class="bg-base-200 rounded-lg p-4 mb-6 flex flex-wrap gap-4 items-center">
        ...
        <div class="ml-auto">
            <button wire:click="saveAllTranslations" class="btn btn-success btn-sm gap-2" wire:loading.attr="disabled">
                ...
                <span wire:loading.remove wire:target="saveAllTranslations">{{ __('Save All') }}</span>
                <span wire:loading wire:target="saveAllTranslations">{{ __('Saving...') }}</span>
            </button>
        </div>
    </div>
    ...
    <div class="space-y-4">
        ...
        @empty
            <div class="text-center py-16">
                ...
                <p class="text-lg font-medium text-base-content/60">{{ __('No translations found') }}</p>
                <p class="text-sm text-base-content/40 mb-4">{{ __('Try adjusting your filters or add a new key') }}</p>
                <button @click="addKeyModal = true; $wire.call('openAddKeyModal')" class="btn btn-primary btn-sm">{{ __('Add Translation Key') }}</button>
            </div>
        @endforelse
    </div>
        ...
        <div class="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4 bg-base-200 rounded-lg p-4">
            ...
            <div class="flex items-center gap-2">
                <div class="join">
                    ...
                    <button class="join-item btn btn-sm">{{ __('Page') }} {{ $this->getPage() }}</button>
    ...
    {{-- Add Key Modal --}}
    <x-modal wire:model="addKeyModal" :title="__('Add Translation Key')" class="backdrop-blur" persistent>
        <div class="space-y-4">
            <x-input :label="__('Translation Key')" wire:model="newKey" :placeholder="__('e.g., welcome_message')" />

            <div class="divider text-sm">{{ __('Translations') }}</div>
    ...
    {{-- Add Language Modal --}}
    <x-modal wire:model="addLanguageModal" :title="__('Add New Language')" :subtitle="__('2-letter ISO 639-1 language code')" class="backdrop-blur" persistent>
        <div class="space-y-4">
            <x-input
                :label="__('Language Code')"
                wire:model="newLanguageCode"
                :placeholder="__('e.g., es, fr, de')"
                maxlength="2"
                :hint="__('2-letter language code')"
            />

            <x-alert icon="o-information-circle" class="alert-info">
                {{ __('English translations will be used as placeholders') }}
            </x-alert>
        </div>

        <x-slot:actions>
            <x-button :label="__('Cancel')" @click="$wire.closeAddLanguageModal()" />
            <x-button :label="__('Add Language')" class="btn-primary" wire:click="saveNewLanguage" spinner="saveNewLanguage" />
        </x-slot:actions>
    </x-modal>

    {{-- Import Modal --}}
    <x-modal wire:model="importModal" :title="__('Import Translations')" class="backdrop-blur max-w-2xl" persistent>
        <div class="space-y-4">
            <x-select
            ...
            <x-alert icon="o-exclamation-triangle" class="alert-warning">
                {{ __('This will overwrite existing translations') }}
            </x-alert>
        </div>

        <x-slot:actions>
            <x-button :label="__('Cancel')" @click="$wire.closeImportModal()" />
            <x-button :label="__('Import')" class="btn-primary" wire:click="importLanguageFile" spinner="importLanguageFile" />
        </x-slot:actions>
    </x-modal>

    {{-- Scan Modal --}}
    <x-modal wire:model="scanModal" :title="__('Scan Code for Translation Keys')" class="backdrop-blur max-w-4xl" persistent>
        <div class="space-y-4">
            <x-alert icon="o-information-circle" class="alert-info">
                {{ __('Scans for __(), trans(), and lang() functions') }}
            </x-alert>

            <x-button :label="__('Start Scan')" class="btn-primary" wire:click="scanForKeys" spinner="scanForKeys" />
        ...
        <x-slot:actions>
            <x-button :label="__('Close')" @click="$wire.closeScanModal()" />
            @if(count($scannedKeys) > 0)
                <x-button :label="__('Sync Keys')" class="btn-primary" wire:click="syncScannedKeys" spinner="syncScannedKeys" />
            @endif
        </x-slot:actions>
    </x-modal>

    {{-- AI Translate Modal --}}
    <x-modal wire:model="aiTranslateModal" :title="__('AI Auto-Translate')" :subtitle="__('Translate missing keys using Google Translate')" class="backdrop-blur" persistent>
        <div class="space-y-4">
            <x-select
            ...
            <x-alert icon="o-information-circle" class="alert-info">
                {{ __('Uses Google Translate - No API key required!') }}
            </x-alert>

            <x-alert icon="o-exclamation-triangle" class="alert-warning">
                {{ __('Translations are processed in batches of 20 to prevent timeouts. This may take a moment.') }}
            </x-alert>
        </div>

        <x-slot:actions>
            <x-button :label="__('Cancel')" @click="$wire.closeAITranslateModal()" />
            <x-button :label="__('Translate')" class="btn-primary" wire:click="autoTranslate" spinner="autoTranslate" />
        </x-slot:actions>
    </x-modal>
