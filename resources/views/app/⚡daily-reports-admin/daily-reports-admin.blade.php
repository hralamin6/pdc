<div class="max-w-7xl mx-auto py-6 space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-base-100 p-6 rounded-3xl border border-base-content/5 shadow-sm">
        ...
        <div class="flex items-center gap-2">
            <x-button 
                ...
                wire:confirm="Send daily report reminder notifications to all pending members?" 
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="bg-base-100 p-5 rounded-2xl border border-base-content/5 shadow-sm flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-xs font-bold text-base-content/50 uppercase">Total Members</p>
            </div>
        </div>
        ...
        <div class="bg-base-100 p-5 rounded-2xl border border-base-content/5 shadow-sm flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-xs font-bold text-base-content/50 uppercase">Submitted Today</p>
            </div>
        </div>
        ...
        <div class="bg-base-100 p-5 rounded-2xl border border-base-content/5 shadow-sm flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-xs font-bold text-base-content/50 uppercase">Pending Today</p>
            </div>
        </div>
        ...
        <div class="bg-base-100 p-5 rounded-2xl border border-base-content/5 shadow-sm flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-xs font-bold text-base-content/50 uppercase">Today's Rate</p>
            </div>
        </div>
    </div>
                        ...
                        <tr class="hover:bg-base-200/30 transition-colors">
                            ...
                            <td>
                                @if($user->daily_reports_count > 0)
                                    <span class="badge badge-success badge-sm font-bold">Submitted</span>
                                @else
                            ...
                            <td class="text-xs text-base-content/60">
                                {{ $user->userStreak?->last_report_date ? $user->userStreak->last_report_date->diffForHumans() : 'Never' }}
                            </td>
                            <td>
    ...
    <x-modal wire:model="inspectingUserId" title="Member Report Inspection" class="backdrop-blur">
        @if($inspectedUser)
            <div class="space-y-6">
                {{-- User summary card --}}
                <div class="flex items-center gap-4 p-4 rounded-2xl bg-base-200/50 border border-base-content/5">
                    ...
                    <div class="text-right">
                        <div class="flex items-center gap-1 font-black text-orange-500 text-sm">
                            ...
                            {{ $inspectedUser['current_streak'] }} Day Streak
                        </div>
                        <p class="text-[11px] text-base-content/50 mt-0.5">Best: {{ $inspectedUser['longest_streak'] }} days</p>
                    </div>
                </div>

                {{-- Recent Logs --}}
                <div class="space-y-3">
                    <h5 class="text-xs font-bold text-base-content/50 uppercase tracking-wider">Recent Reports (Past 7 Days)</h5>
                    
                    @if(empty($inspectedUser['recent_reports']))
                        <p class="text-xs text-base-content/40 italic py-4 text-center">No submitted reports found for this member yet.</p>
                    @else
                            ...
                            @foreach($inspectedUser['recent_reports'] as $report)
                                <div class="p-3 rounded-xl bg-base-200/30 border border-base-content/5 space-y-2">
                                    <div class="flex items-center justify-between text-xs">
                                        ...
                                        <span class="badge badge-sm badge-success font-bold">{{ $report['completion_percentage'] }}% Done</span>
                                    </div>
                                    <div class="flex items-center justify-between text-[11px] text-base-content/60">
                                        <span>Items: <strong>{{ $report['completed_count'] }}/{{ $report['total_items_count'] }}</strong></span>
                                        <span class="capitalize">Privacy: {{ $report['privacy_level'] }}</span>
                                    </div>
        ...
        <x-slot:actions>
            <x-button label="Close" wire:click="closeInspectModal" class="btn-ghost rounded-xl" />
        </x-slot:actions>
    </x-modal>
</div>
