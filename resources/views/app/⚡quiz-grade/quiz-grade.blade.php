<div>
    <x-header title="Grade Short Answers" subtitle="Review AI-graded short text answers and confirm them." separator>
        <x-slot:actions>
            <x-select wire:model.live="quizId" :options="$this->quizzes" option-label="title" option-value="id" placeholder="All Quizzes" class="select-bordered select-sm max-w-xs" />
            
            <div class="join">
                <button wire:click="$set('filterStatus', 'pending')" class="btn btn-sm join-item {{ $filterStatus === 'pending' ? 'btn-primary' : '' }}">Pending</button>
                <button wire:click="$set('filterStatus', 'graded')" class="btn btn-sm join-item {{ $filterStatus === 'graded' ? 'btn-primary' : '' }}">Graded</button>
            </div>
    ...
    @if($this->answers->isEmpty())
        <div class="py-12 text-center text-base-content/50">
            ...
            <p>No {{ $filterStatus }} answers found.</p>
        </div>
    @else
        <div class="space-y-4">
                    ...
                    <div class="flex items-start justify-between gap-4 mb-4">
                        <div>
                            <div class="flex items-center gap-2 mb-1">
                                ...
                                <span class="text-xs text-base-content/50">in {{ $ans->attempt->quiz->title }}</span>
                            ...
                            <p class="text-xs text-base-content/50 mt-1">{{ $ans->question->marks }} max marks</p>
                        </div>
                        ...
                        <div class="flex items-center gap-2 bg-base-200/50 p-2 rounded-lg border border-base-content/10">
                            <div class="text-xs text-right mr-2">
                                <span class="block text-base-content/50">AI Grade</span>
                            ...
                            <div class="border-l border-base-content/10 pl-3">
                                <label class="text-[10px] font-bold text-base-content/50 block mb-1">Admin Grade (0-1)</label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div class="bg-info/5 rounded-lg p-3 border border-info/10">
                            <p class="text-xs font-bold text-info uppercase mb-2 flex items-center gap-1">
                                <x-icon name="o-sparkles" class="w-3 h-3" /> AI Reasoning
                            </p>
                            <p class="text-sm text-base-content/70 italic">{{ $ans->ai_grade_reason ?? 'No AI reasoning available.' }}</p>
                        </div>
                    </div>
                </div>
            @endforeach
