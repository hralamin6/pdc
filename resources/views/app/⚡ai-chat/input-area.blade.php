...
<div class="p-3 md:p-4 bg-base-100 border-t border-base-300">
    ...
    @if($editingMessageId)
        <div class="mb-3 p-3 bg-info/10 border-l-4 border-info rounded">
            <div class="flex items-center justify-between">
                <span class="text-sm text-info-content">{{ __('Editing message') }}</span>
    ...
    <form wire:submit="{{ $editingMessageId ? 'updateMessage' : 'sendMessage' }}" class="flex items-end gap-2">
        {{-- File Upload --}}
        <label class="flex-shrink-0 cursor-pointer btn btn-ghost btn-sm btn-circle" title="{{ __('Upload images (Vision: Gemini, Mistral, Groq, Pollinations)') }}">
        ...
        <textarea
            wire:model="message"
            placeholder="{{ __("Type your message... (Images work with vision models marked 'img')") }}"
            rows="1"
    ...
    <p class="text-xs text-base-content/60 mt-2">
        {{ __('Press Enter to send, Shift+Enter for new line. 📷 Vision + 🎨 Image Gen: Pollinations!') }}
    </p>
</div>
