<div>
    <x-header :title="__('⚡ Host Control — :title', ['title' => $quiz->title])" subtitle="{{ __('Live quiz control panel') }}" separator>
        <x-slot:actions>
            <x-button icon="o-arrow-path" label="{{ __('Refresh') }}" wire:click="refreshStats" class="btn-ghost btn-sm" spinner="refreshStats" />
            @if($quiz->status === 'live')
                <x-button icon="o-signal" label="{{ __('Push Leaderboard') }}" wire:click="broadcastLeaderboard" class="btn-primary btn-sm" spinner="broadcastLeaderboard" />
                <x-button icon="o-stop" label="{{ __('Force End Quiz') }}" wire:click="$set('confirmEndModal', true)" class="btn-error btn-sm" />
            @elseif($quiz->status === 'published')
                <x-button icon="o-play" label="{{ __('Start Live Quiz') }}" wire:click="startLiveQuiz" class="btn-success shadow-lg shadow-success/30" spinner="startLiveQuiz" />
            @endif
        </x-slot:actions>
    </x-header>
    ...
    >
        ...
        @if($quiz->status === 'live')
            {{ __('LIVE — Started :time', ['time' => $quiz->live_started_at?->diffForHumans()]) }}
        @elseif($quiz->status === 'closed')
            {{ __('CLOSED — Quiz ended') }}
        @else
            {{ __('READY — Click "Start Live Quiz" when participants have joined') }}
            @if($quiz->available_from && $quiz->available_from > now())
                <span class="ml-2 font-black text-warning">{{ __('Auto-starting in:') }} <span x-text="format()" class="font-mono bg-warning/20 px-2 py-0.5 rounded ml-1"></span></span>
            @endif
        @endif
    </div>
        ...
        <div class="lg:col-span-2 space-y-5">
            ...
            <div class="grid grid-cols-3 gap-4">
                <div class="bg-base-100 rounded-2xl p-4 text-center border border-base-content/5">
                    ...
                    <div class="text-xs text-base-content/50 mt-1">{{ __('Total Joined') }}</div>
                </div>
                <div class="bg-base-100 rounded-2xl p-4 text-center border border-base-content/5">
                    <div class="text-3xl font-black text-warning">{{ count($participants) }}</div>
                    <div class="text-xs text-base-content/50 mt-1">{{ __('In Progress') }}</div>
                </div>
                <div class="bg-base-100 rounded-2xl p-4 text-center border border-base-content/5">
                    <div class="text-3xl font-black text-success">{{ $submittedCount }}</div>
                    <div class="text-xs text-base-content/50 mt-1">{{ __('Submitted') }}</div>
                </div>
            </div>
            ...
            <div class="bg-base-100 rounded-2xl border border-base-content/5 p-5">
                <h3 class="font-bold mb-4 flex items-center gap-2">
                    <x-icon name="o-chart-bar" class="w-4 h-4 text-primary" /> {{ __('Answer Progress by Question') }}
                </h3>
                <div class="space-y-3">
            ...
            <div class="bg-base-100 rounded-2xl border border-base-content/5 p-5">
                <h3 class="font-bold mb-4 flex items-center gap-2">
                    <x-icon name="o-users" class="w-4 h-4 text-warning" /> {{ __('Currently Taking (:count)', ['count' => count($participants)]) }}
                </h3>
    ...
    <x-modal wire:model="confirmEndModal" :title="__('Force End Quiz?')" box-class="max-w-sm">
        <div class="text-center space-y-3 py-2">
            <div class="text-5xl">⚠️</div>
            <p class="font-bold">{{ __('This will auto-submit all :count in-progress attempt(s) and close the quiz.', ['count' => count($participants)]) }}</p>
            <p class="text-sm text-base-content/50">{{ __('This action cannot be undone.') }}</p>
        </div>
        <x-slot:actions>
            <x-button label="{{ __('Cancel') }}" wire:click="$set('confirmEndModal', false)" class="btn-ghost" />
            <x-button label="{{ __('End Quiz Now') }}" icon="o-stop" wire:click="forceEndQuiz" class="btn-error" spinner="forceEndQuiz" />
        </x-slot:actions>
    </x-modal>
