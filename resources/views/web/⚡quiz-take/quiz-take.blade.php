    ...
    <div class="bg-base-100 rounded-3xl border border-base-content/5 shadow-xl overflow-hidden">
        ...
        <div class="p-8">
            ...
            <div class="flex items-center gap-3 mb-5">
                <span class="badge badge-primary border-none font-bold">{{ $quiz->mode === 'live' ? __('⚡ Live Quiz') : __('📝 Async Quiz') }}</span>
                @if($quiz->negative_marking)
                    <span class="badge badge-error border-none text-xs">{{ __('⚠ Negative Marking') }}</span>
                @endif
            </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                <div class="bg-base-200/50 rounded-xl p-3 text-center">
                    ...
                    <div class="text-xs text-base-content/50 mt-0.5">{{ __('Questions') }}</div>
                </div>
                <div class="bg-base-200/50 rounded-xl p-3 text-center">
                    <div class="text-2xl font-black text-secondary">{{ $quiz->total_marks }}</div>
                    <div class="text-xs text-base-content/50 mt-0.5">{{ __('Total Marks') }}</div>
                </div>
                <div class="bg-base-200/50 rounded-xl p-3 text-center">
                    ...
                    @endif
                    <div class="text-xs text-base-content/50 mt-0.5">{{ __('Time Limit') }}</div>
                </div>
                <div class="bg-base-200/50 rounded-xl p-3 text-center">
                    ...
                    @endif
                    <div class="text-xs text-base-content/50 mt-0.5">{{ __('Pass Mark') }}</div>
                </div>
            </div>
            ...
            <div class="space-y-2 mb-8">
                @if($quiz->shuffle_questions)
                    <div class="flex items-center gap-2 text-sm text-base-content/60">
                        <x-icon name="o-arrows-up-down" class="w-4 h-4 flex-shrink-0 text-primary/60" /> {{ __('Questions are shuffled for each attempt') }}
                    </div>
                @endif
                @if($quiz->negative_marking)
                    <div class="flex items-center gap-2 text-sm text-error">
                        <x-icon name="o-minus-circle" class="w-4 h-4 flex-shrink-0" /> {{ __('Wrong answers deduct') }} <strong>{{ $quiz->negative_mark_value * 100 }}%</strong> {{ __('of the question marks') }}
                    </div>
                @endif
                @if($quiz->show_answers_after)
                    <div class="flex items-center gap-2 text-sm text-base-content/60">
                        <x-icon name="o-eye" class="w-4 h-4 flex-shrink-0 text-success/60" /> {{ __('Correct answers revealed after submission') }}
                    </div>
                @endif
                <div class="flex items-center gap-2 text-sm text-base-content/60">
                    <x-icon name="o-lock-closed" class="w-4 h-4 flex-shrink-0 text-warning/60" /> {{ __('One attempt only — cannot retake') }}
                </div>
            </div>

            <x-button :label="__('Start Quiz')" icon="o-play" class="btn-primary w-full text-lg py-4 rounded-2xl shadow-lg shadow-primary/30 hover:scale-[1.02] transition-transform" wire:click="startQuiz" spinner="startQuiz" />
        </div>
    </div>
</div>
...
<div class="max-w-3xl mx-auto"
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="flex-grow">
                <div class="flex items-center justify-between mb-1">
                    <span class="text-xs font-bold text-base-content/50">{{ __('Q') }} {{ $currentIndex + 1 }} / {{ $totalQuestions }}</span>
                    <span class="text-xs text-base-content/50">{{ $answeredCount }} {{ __('answered') }}</span>
                </div>
            ...
            {{-- Navigator toggle --}}
            <button wire:click="$toggle('showNavigator')" class="btn btn-ghost btn-sm btn-circle" :title="__('Question grid')">
    ...
    <div class="bg-base-100 rounded-2xl border border-base-content/10 p-4 mb-4 shadow-xl">
        <div class="flex items-center justify-between mb-3">
            <h3 class="font-bold text-sm">{{ __('Question Navigator') }}</h3>
            <div class="flex items-center gap-3 text-xs text-base-content/50">
                <span class="flex items-center gap-1"><span class="w-3 h-3 rounded-full bg-primary inline-block"></span> {{ __('Answered') }}</span>
                <span class="flex items-center gap-1"><span class="w-3 h-3 rounded-full bg-warning inline-block"></span> {{ __('Flagged') }}</span>
                <span class="flex items-center gap-1"><span class="w-3 h-3 rounded-full bg-base-300 inline-block"></span> {{ __('Unanswered') }}</span>
            </div>
        </div>
    ...
    <div class="bg-base-100 rounded-2xl border border-base-content/5 shadow-sm overflow-hidden">
        ...
        <div class="flex items-center justify-between px-6 py-3 bg-base-200/50 border-b border-base-content/5">
            <div class="flex items-center gap-2">
                ...
                <span class="text-xs text-base-content/40">{{ $q->marks }} {{ $q->marks == 1 ? __('mark') : __('marks') }}</span>
                @if($quiz->negative_marking && $q->type !== 'short_text')
                    <span class="text-xs text-error">(-{{ $q->marks * $quiz->negative_mark_value }} {{ __('wrong') }})</span>
                @endif
            </div>
            <button wire:click="toggleFlag" class="btn btn-ghost btn-xs gap-1 {{ ($flagged[$qid] ?? false) ? 'text-warning' : 'text-base-content/30' }}">
                ...
                {{ ($flagged[$qid] ?? false) ? __('Flagged') : __('Flag') }}
            </button>
        </div>
        ...
        <div class="p-6 space-y-5">
            ...
            @if($q->type !== 'short_text')
                @if($q->type === 'multi_select')
                    <p class="text-xs text-info font-medium">{{ __('Select all that apply') }}</p>
                @endif
                <div class="space-y-2">
            ...
            @else
                {{-- Short text answer --}}
                <div>
                    <p class="text-xs text-base-content/40 mb-2">{{ __('Type your answer below. It will be reviewed by AI and admin.') }}</p>
    ...
    <div class="flex items-center justify-between mt-5">
        <x-button icon="o-arrow-left" :label="__('Previous')" class="btn-ghost" wire:click="prevQuestion"
            :disabled="$currentIndex === 0" />

        <div class="flex gap-2">
            @if($currentIndex < $totalQuestions - 1)
                <x-button icon-right="o-arrow-right" :label="__('Next')" class="btn-primary" wire:click="nextQuestion" />
            @else
                <x-button icon="o-check-circle" :label="__('Submit Quiz')" class="btn-success shadow-lg shadow-success/30" wire:click="confirmSubmit" />
            @endif
        </div>
    </div>
    @endif
    ...
    <x-modal wire:model="showSubmitConfirm" :title="__('Submit Quiz?')" box-class="max-w-sm">
        <div class="text-center space-y-4">
            <div class="text-6xl">📋</div>
            <div class="space-y-1">
                <p class="font-bold text-lg">{{ $answeredCount }} / {{ $totalQuestions }} {{ __('questions answered') }}</p>
                @if($unansweredCount > 0)
                    <p class="text-warning text-sm font-medium">{{ $unansweredCount }} {{ __('unanswered question(s) will score 0.') }}</p>
                @else
                    <p class="text-success text-sm font-medium">{{ __('All questions answered!') }}</p>
                @endif
            </div>
            <p class="text-sm text-base-content/50">{{ __('This cannot be undone. Are you ready to submit?') }}</p>
        </div>
        <x-slot:actions>
            <x-button :label="__('Keep Reviewing')" wire:click="$set('showSubmitConfirm', false)" class="btn-ghost" />
            <x-button :label="__('Submit Now')" icon="o-check" class="btn-success" wire:click="submitQuiz" spinner="submitQuiz" />
        </x-slot:actions>
    </x-modal>
</div>
...
<div class="max-w-3xl mx-auto space-y-6">
        ...
        <div class="p-8 text-center">
            ...
            @if(!is_null($results['passed']))
                <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full font-bold text-sm mb-4
                    ...
                    {{ $results['passed'] ? __('PASSED') : __('NOT PASSED') }}
                    @if($results['pass_mark_percent'])
                        ({{ __('pass mark:') }} {{ $results['pass_mark_percent'] }}%)
                    @endif
                </div>
            @endif
            ...
            @if($results['status'] === 'timed_out')
                <div class="mt-4 p-2 rounded-lg bg-warning/10 text-warning text-sm font-medium">
                    ⏰ {{ __('Time expired — quiz was auto-submitted') }}
                </div>
            @endif
        </div>
    </div>
    ...
    <div>
        <h2 class="font-black text-lg mb-4 flex items-center gap-2">
            <x-icon name="o-clipboard-document-list" class="w-5 h-5 text-primary" /> {{ __('Answer Breakdown') }}
        </h2>
        <div class="space-y-3">
            ...
            <div class="bg-base-100 rounded-2xl border border-base-content/5 overflow-hidden">
                <div class="flex items-center gap-3 px-4 py-2.5 border-b border-base-content/5
                    ...
                    <div class="flex items-center gap-2 flex-shrink-0">
                        ...
                        @else
                            <span class="badge badge-warning text-xs">{{ __('Pending Grade') }}</span>
                        @endif
                ...
                <div class="px-4 py-3 space-y-1.5">
                    @foreach($qr['options'] as $opt)
                        ...
                        <div class="flex items-center gap-2 text-sm rounded-lg px-2 py-1
                            ...
                            @endif
                            {{ $opt['option_text'] }}
                            @if($wasSelected && !$isCorrect) <span class="text-xs">{{ __('(your answer)') }}</span> @endif
                        </div>
                    @endforeach
                </div>
                @endif
                ...
                {{-- Short text answer --}}
                @if($qr['type'] === 'short_text')
                <div class="px-4 py-3 space-y-3">
                    @if(!empty($qr['ideal_answer']))
                        <div>
                            <p class="text-xs font-bold text-success mb-1">{{ __('Ideal Answer:') }}</p>
                            <p class="text-sm italic text-base-content/70">{{ $qr['ideal_answer'] }}</p>
                        </div>
                    @endif
                    <div>
                        <p class="text-xs font-bold text-base-content/40 mb-1">{{ __('Your answer:') }}</p>
                        @if($qr['text_answer'])
                            <p class="text-sm italic">{{ $qr['text_answer'] }}</p>
                            @if($qr['ai_grade_reason'])
                                <p class="text-xs text-info mt-1"><span class="font-bold">{{ __('AI Feedback:') }}</span> {{ $qr['ai_grade_reason'] }}</p>
                            @endif
                        @else
                            <p class="text-xs text-warning">{{ __('No answer provided.') }}</p>
                        @endif
                    </div>
                </div>
                @endif
                ...
                <div class="px-4 py-3 bg-info/5 border-t border-info/10">
                    <p class="text-xs font-bold text-info mb-1 flex items-center gap-1">
                        <x-icon name="o-sparkles" class="w-3 h-3" /> {{ __('Explanation') }}
                    </p>
    ...
    <div class="flex gap-3 justify-center pb-8">
        <x-button :label="__('Back')" icon="o-arrow-left" class="btn-ghost" :href="$backUrl" wire:navigate />
    </div>
</div>
@endif
