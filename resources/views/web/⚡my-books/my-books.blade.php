<div class="bg-slate-50/40 dark:bg-slate-950/40 min-h-screen py-10">
            ...
            <div class="flex items-center gap-3">
                @if($activeTab === 'copies')
                    <button wire:click="openAddModal" class="btn btn-primary btn-sm rounded-xl px-5 font-bold shadow-lg shadow-primary/25 hover:scale-105 transition-transform">
                        <x-icon name="o-plus" class="w-4 h-4" /> Add Book to Shelf
                    </button>
                @elseif($activeTab === 'reading')
                    <button wire:click="openAddReadingModal" class="btn btn-primary btn-sm rounded-xl px-5 font-bold shadow-lg shadow-primary/25 hover:scale-105 transition-transform">
                        <x-icon name="o-plus" class="w-4 h-4" /> Track a Book
                    </button>
                @endif
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    ...
                    <div wire:key="copy-{{ $copy->id }}" class="group bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/50 dark:border-slate-800/80 overflow-hidden flex flex-col transition-all duration-300 hover:shadow-xl hover:shadow-slate-200/30 dark:hover:shadow-none hover:border-slate-300 dark:hover:border-slate-700">
                        ...
                        <div class="h-56 bg-slate-100 dark:bg-slate-950 relative overflow-hidden flex items-center justify-center">
                            ...
                            @else
                                <div class="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-indigo-50/50 to-slate-100 dark:from-slate-800 dark:to-slate-900 text-slate-300 dark:text-slate-700">
                                    ...
                                    <span class="text-xs uppercase font-semibold text-slate-400">No Cover Available</span>
                                </div>
                            @endif
                            ...
                            <div class="absolute top-3 right-3 flex flex-col gap-1">
                                @if($copy->status === 'available')
                                    <span class="px-2.5 py-0.5 text-[10px] font-black uppercase bg-emerald-500 text-white rounded-lg shadow-sm">Available</span>
                                @elseif($copy->status === 'borrowed')
                                    <span class="px-2.5 py-0.5 text-[10px] font-black uppercase bg-amber-500 text-white rounded-lg shadow-sm">Borrowed</span>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="p-5 flex-grow flex flex-col justify-between">
                            <div>
                                ...
                                <p class="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-4">{{ $copy->book->author?->name ?? 'Unknown Author' }}</p>
                                ...
                                <div class="flex items-center justify-between text-xs font-semibold text-slate-600 dark:text-slate-400 mb-4 bg-slate-50 dark:bg-slate-950 p-2.5 rounded-xl border border-slate-200/50 dark:border-slate-800/80">
                                    <span>Condition: <span class="text-slate-800 dark:text-slate-200 font-bold">{{ $copy->condition }}</span></span>
                                    <span>
                                        @if($copy->is_borrowable)
                                            <span class="text-emerald-600 dark:text-emerald-400 flex items-center gap-1"><x-icon name="o-check-circle" class="w-3.5 h-3.5" /> Lendable</span>
                                        @else
                                            <span class="text-slate-400 flex items-center gap-1"><x-icon name="o-lock-closed" class="w-3.5 h-3.5" /> Private</span>
                                        @endif
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="pt-3 border-t border-slate-100 dark:border-slate-850 flex justify-between items-center">
                                <button wire:click="toggleBorrowable({{ $copy->id }})" class="btn btn-ghost btn-xs text-xs font-bold rounded-lg px-2.5 py-1 {{ $copy->is_borrowable ? 'text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/20' : 'text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-950/20' }}">
                                    {{ $copy->is_borrowable ? 'Make Private' : 'Make Lendable' }}
                                </button>

                                <button wire:click="deleteCopy({{ $copy->id }})" wire:confirm="Remove this copy from your shelf? This action cannot be undone." class="btn btn-ghost btn-circle btn-xs text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/20">
                ...
                @empty
                    <div class="col-span-full text-center py-20 bg-white dark:bg-slate-900 rounded-3xl border border-dashed border-slate-200 dark:border-slate-800 p-8 shadow-sm">
                        ...
                        <h3 class="text-xl font-black text-slate-800 dark:text-slate-200 mb-2">Your shelf is empty</h3>
                        <p class="text-sm text-slate-450 dark:text-slate-500 max-w-sm mx-auto mb-6">List physical books you own to let others in the community borrow them, or catalog them on your private shelf.</p>
                        <button wire:click="openAddModal" class="btn btn-primary btn-sm rounded-xl px-5 font-bold shadow-lg shadow-primary/25">
                            <x-icon name="o-plus" class="w-4 h-4" /> Add Your First Book
                        </button>
                    </div>
                @endforelse
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div wire:key="interaction-{{ $interaction->id }}" class="group bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/50 dark:border-slate-800/80 p-5 flex gap-4 transition-all duration-300 hover:shadow-xl hover:shadow-slate-200/30 dark:hover:shadow-none hover:border-slate-300 dark:hover:border-slate-700">
                        ...
                        <div class="flex-grow flex flex-col justify-between">
                            <div>
                                ...
                                <p class="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-2">{{ $interaction->book->author?->name ?? 'Unknown Author' }}</p>

                                @if($interaction->reading_status === 'reading')
                                    <div class="mt-2.5">
                                        <div class="flex items-center justify-between text-[11px] font-bold text-slate-500 mb-1">
                                            <span>Progress</span>
                                            <span>{{ $interaction->pages_read }} / {{ $interaction->book->pages_count ?? '?' }} pages</span>
                                        </div>
                                ...
                                @elseif($interaction->reading_status === 'completed')
                                    <div class="flex items-center gap-1 mt-2.5">
                                        ...
                                        @else
                                            <span class="text-[10px] font-bold text-slate-400 uppercase">Not Rated Yet</span>
                                        @endif
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="flex items-center justify-between mt-3 pt-2.5 border-t border-slate-100 dark:border-slate-850">
                                <button wire:click="openProgressModal({{ $interaction->id }})" class="btn btn-ghost btn-xs text-xs font-bold text-primary hover:bg-primary/5 rounded-lg px-2">
                                    <x-icon name="o-pencil" class="w-3 h-3" /> Update Log
                                </button>

                                <button wire:click="removeInteraction({{ $interaction->id }})" wire:confirm="Remove this book from your reading shelf?" class="btn btn-ghost btn-circle btn-xs text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/20">
                ...
                @empty
                    <div class="col-span-full text-center py-20 bg-white dark:bg-slate-900 rounded-3xl border border-dashed border-slate-200 dark:border-slate-800 p-8 shadow-sm">
                        ...
                        <h3 class="text-xl font-black text-slate-800 dark:text-slate-200 mb-2">No books in this list</h3>
                        <p class="text-sm text-slate-450 dark:text-slate-500 max-w-sm mx-auto mb-6">Search our catalog and add books to keep track of your reading wishlist and log pages completed.</p>
        ...
        @if($activeTab === 'requests')
            ...
            <div class="flex items-center gap-1.5 mb-6">
                ...
                <button wire:click="switchRequestsTab('outgoing')" class="px-4 py-1.5 text-xs font-black rounded-xl border transition {{ $requestsSubTab === 'outgoing' ? 'bg-primary border-primary text-white shadow-sm' : 'bg-white dark:bg-slate-900 border-slate-200/60 dark:border-slate-800/80 text-slate-600 dark:text-slate-400 hover:border-slate-350' }}">
                    📤 Outgoing Requests (Borrow Books)
                </button>
            </div>
                        ...
                        <table class="w-full text-left text-sm">
                            <thead>
                                <tr class="bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800/80 text-slate-500 text-xs font-black uppercase tracking-wider">
                                    <th class="py-4 px-6">Book Copy Details</th>
                                    <th class="py-4 px-6">Requester</th>
                                    <th class="py-4 px-6">Days Requested</th>
                            ...
                            <tbody class="divide-y divide-slate-100 dark:divide-slate-850">
                                @forelse($this->incomingRequests as $req)
                                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-950/20 transition-colors" wire:key="incoming-{{ $req->id }}">
                                        <td class="py-4 px-6">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <span class="text-xs text-slate-500">Condition: <span class="font-semibold">{{ $req->bookCopy->condition }}</span></span>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="py-4 px-6">
                                        ...
                                        <td class="py-4 px-6 font-semibold text-slate-600 dark:text-slate-400">{{ $req->requested_days }} days</td>
                                        <td class="py-4 px-6">
                                            @if($req->status === 'pending')
                                                <span class="px-2 py-0.5 rounded-lg text-xs font-black uppercase bg-blue-50 text-blue-600 dark:bg-blue-950/20 dark:text-blue-400">Pending</span>
                                            @elseif($req->status === 'accepted')
                                                <span class="px-2 py-0.5 rounded-lg text-xs font-black uppercase bg-amber-50 text-amber-600 dark:bg-amber-950/20 dark:text-amber-400">Accepted</span>
                                            @elseif($req->status === 'rejected')
                                                <span class="px-2 py-0.5 rounded-lg text-xs font-black uppercase bg-rose-50 text-rose-600 dark:bg-rose-950/20 dark:text-rose-400">Rejected</span>
                                            @elseif($req->status === 'given')
                                                <span class="px-2 py-0.5 rounded-lg text-xs font-black uppercase bg-violet-50 text-violet-600 dark:bg-violet-950/20 dark:text-violet-400">Handed Over</span>
                                            @elseif($req->status === 'active')
                                                <span class="px-2 py-0.5 rounded-lg text-xs font-black uppercase bg-emerald-50 text-emerald-600 dark:bg-emerald-950/20 dark:text-emerald-400">On Loan</span>
                                            @elseif($req->status === 'returned')
                                                <span class="px-2 py-0.5 rounded-lg text-xs font-black uppercase bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400">Returned</span>
                                            @endif
                                        </td>
                                        <td class="py-4 px-6 text-right">
                                            <div class="flex items-center justify-end gap-1.5">
                                                @if($req->status === 'pending')
                                                    <button wire:click="acceptRequest({{ $req->id }})" class="btn btn-emerald btn-xs rounded-lg font-bold">Accept</button>
                                                    <button wire:click="rejectRequest({{ $req->id }})" class="btn btn-ghost btn-xs text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/20 rounded-lg font-bold">Reject</button>
                                                @elseif($req->status === 'accepted')
                                                    <button wire:click="markGiven({{ $req->id }})" class="btn btn-primary btn-xs rounded-lg font-bold">Hand Over</button>
                                                @elseif($req->status === 'active' || $req->status === 'given')
                                                    @if($req->status === 'active')
                                                        <button wire:click="sendReminder({{ $req->id }})" class="btn btn-warning btn-xs rounded-lg font-bold">Remind</button>
                                                    @endif
                                                    <button wire:click="confirmReturned({{ $req->id }})" wire:confirm="Confirm that you have physically received this book back?" class="btn btn-emerald btn-xs rounded-lg font-bold">Mark Returned</button>
                                                @else
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="text-center py-16 text-slate-450 dark:text-slate-500 font-bold">No incoming lend requests found.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                        ...
                        <table class="w-full text-left text-sm">
                            <thead>
                                <tr class="bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800/80 text-slate-500 text-xs font-black uppercase tracking-wider">
                                    ...
                                    <th class="py-4 px-6">Due Date / Days</th>
                                    <th class="py-4 px-6">Status</th>
                                    <th class="py-4 px-6 text-right">Actions</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-slate-100 dark:divide-slate-850">
                                @forelse($this->outgoingRequests as $req)
                                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-950/20 transition-colors" wire:key="outgoing-{{ $req->id }}">
                                        ...
                                        <td class="py-4 px-6">
                                            <div class="flex items-center gap-2">
                                                <div class="w-7 h-7 rounded-xl bg-slate-100 dark:bg-slate-850 ring-2 ring-primary/10 overflow-hidden flex items-center justify-center text-[10px] font-black text-primary">
                                                    ...
                                                    @else
                                                        {{ substr($req->bookCopy->owner->name ?? 'O', 0, 1) }}
                                                    @endif
                                                </div>
                                                <span class="font-semibold text-slate-700 dark:text-slate-350">{{ $req->bookCopy->owner->name }}</span>
                                            </div>
                                        </td>
                                        ...
                                        <td class="py-4 px-6">
                                            @if($req->status === 'pending')
                                                <span class="px-2 py-0.5 rounded-lg text-xs font-black uppercase bg-blue-50 text-blue-600 dark:bg-blue-950/20 dark:text-blue-400">Waiting for Owner</span>
                                            @elseif($req->status === 'accepted')
                                                <span class="px-2 py-0.5 rounded-lg text-xs font-black uppercase bg-amber-50 text-amber-600 dark:bg-amber-950/20 dark:text-amber-400">Ready to Collect</span>
                                            @elseif($req->status === 'rejected')
                                                <span class="px-2 py-0.5 rounded-lg text-xs font-black uppercase bg-rose-50 text-rose-600 dark:bg-rose-950/20 dark:text-rose-400">Rejected</span>
                                            @elseif($req->status === 'given')
                                                <span class="px-2 py-0.5 rounded-lg text-xs font-black uppercase bg-violet-50 text-violet-600 dark:bg-violet-950/20 dark:text-violet-400">Owner Handed Over</span>
                                            @elseif($req->status === 'active')
                                                <span class="px-2 py-0.5 rounded-lg text-xs font-black uppercase bg-emerald-50 text-emerald-600 dark:bg-emerald-950/20 dark:text-emerald-400">Reading Now</span>
                                            @elseif($req->status === 'returned')
                                                <span class="px-2 py-0.5 rounded-lg text-xs font-black uppercase bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400">Returned</span>
                                            @endif
                                        </td>
                                        <td class="py-4 px-6 text-right">
                                            <div class="flex items-center justify-end gap-1.5">
                                                @if($req->status === 'given')
                                                    <button wire:click="confirmReceived({{ $req->id }})" class="btn btn-emerald btn-xs rounded-lg font-bold">Confirm Received</button>
                                                @elseif($req->status === 'pending' || $req->status === 'accepted')
                                                    <button wire:click="cancelRequest({{ $req->id }})" wire:confirm="Cancel your borrow request?" class="btn btn-ghost btn-xs text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/20 rounded-lg font-bold">Cancel</button>
                                                @else
                                                    <span class="text-xs text-slate-400 font-bold">-</span>
                                                @endif
                                            </div>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="text-center py-16 text-slate-450 dark:text-slate-500 font-bold">No borrow requests sent.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
    ...
    <x-modal wire:model="addModal" title="Add Book to Your Physical Shelf" class="backdrop-blur-sm" box-class="rounded-3xl border border-slate-200/60 dark:border-slate-800/80 bg-white dark:bg-slate-900 shadow-2xl p-6">
        <div class="space-y-4">
            <div>
                <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Search Global Catalog</label>
                <div class="relative">
                    <x-icon name="o-magnifying-glass" class="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-450" />
                    <input type="text" wire:model.live.debounce.300ms="search" placeholder="Type book title..."
                ...
                @forelse($this->availableBooks as $b)
                    <button type="button" wire:click="selectBook({{ $b->id }})" class="w-full text-left p-2.5 rounded-xl flex items-center gap-3 transition-colors {{ $selectedBookId === $b->id ? 'bg-primary text-white shadow-sm' : 'hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300' }}">
                        ...
                        <div>
                            ...
                            <div class="text-xs opacity-70">{{ $b->author?->name ?? 'Unknown Author' }}</div>
                        </div>
                    </button>
                @empty
                    <div class="p-4 text-center text-xs text-slate-400 font-bold uppercase tracking-wider">
                        @if($search)
                            No match found in our global catalog.
                        @else
                            Type to search approved books...
                        @endif
                    </div>
                @endforelse
            </div>
            ...
            @if(!$isCustom)
                <div class="text-center mt-2">
                    <button type="button" wire:click="toggleCustom" class="text-xs font-black text-primary hover:underline uppercase tracking-wider">
                        Can't find your book? Create a new one
                    </button>
                </div>
            @endif
            ...
            @if($isCustom)
                <div class="bg-slate-50/50 dark:bg-slate-950/40 p-5 rounded-2xl border border-slate-200/50 dark:border-slate-800/80 space-y-4 mt-4">
                    <h3 class="font-black text-slate-800 dark:text-slate-200 text-xs uppercase tracking-wider border-b border-slate-100 dark:border-slate-900 pb-2">Create Custom Catalog Entry</h3>
                    
                    <div>
                        <label class="block text-xs font-bold text-slate-450 dark:text-slate-400 mb-1.5">Book Title *</label>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-450 dark:text-slate-400 mb-1.5">Author Name</label>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-bold text-slate-450 dark:text-slate-400 mb-1.5">Category</label>
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-450 dark:text-slate-400 mb-1.5">Total Pages</label>
                    ...
                    {{-- Cover Upload --}}
                    <div>
                        <label class="block text-xs font-bold text-slate-455 dark:text-slate-400 mb-1.5">Book Cover Image</label>
                        <x-file wire:model="customCoverFile" accept="image/*" class="w-full" crop-after-change>
                            <div class="flex flex-col items-center justify-center p-4 border border-dashed border-slate-200 dark:border-slate-800 rounded-2xl hover:bg-slate-100 dark:hover:bg-slate-850 transition cursor-pointer">
                                ...
                                @else
                                    ...
                                    <span class="text-xs text-slate-450 font-semibold">Click to upload cover image</span>
                                @endif
                            </div>
                        </x-file>
                    </div>
            ...
            @if($selectedBookId || $isCustom)
                <div class="grid grid-cols-2 gap-4 mt-4 bg-slate-50/50 dark:bg-slate-950/20 p-4 rounded-2xl border border-slate-200/50 dark:border-slate-800/80">
                    <div>
                        <label class="block text-xs font-bold text-slate-450 dark:text-slate-400 mb-1.5">Physical Condition *</label>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-450 dark:text-slate-400 mb-1.5">Lending Status *</label>
                        <select wire:model="is_borrowable" class="select select-sm select-bordered w-full rounded-xl text-xs bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300">
                            <option value="1">Available to Lend</option>
        ...
        <x-slot:actions>
            <button type="button" wire:click="$set('addModal', false)" class="btn btn-ghost btn-sm rounded-xl font-bold">Cancel</button>
    ...
    <x-modal wire:model="addReadingModal" title="Track a Book on your Reading Shelf" class="backdrop-blur-sm" box-class="rounded-3xl border border-slate-200/60 dark:border-slate-800/80 bg-white dark:bg-slate-900 shadow-2xl p-6">
        <div class="space-y-4">
            <div>
                <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Search Global Catalog</label>
                <div class="relative">
                    <x-icon name="o-magnifying-glass" class="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-450" />
                    <input type="text" wire:model.live.debounce.300ms="searchCatalog" placeholder="Type book title..."
                        class="w-full pl-10 pr-4 py-2.5 text-sm bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl focus:ring-2 focus:ring-primary/45 focus:border-primary/45 focus:outline-none transition text-slate-700 dark:text-slate-350" />
                </div>
            </div>
                ...
                @forelse($this->availableBooks as $b)
                    <div class="w-full p-2 rounded-xl flex items-center justify-between gap-3 hover:bg-slate-100 dark:hover:bg-slate-850 text-slate-700 dark:text-slate-300">
                        ...
                        <div class="flex gap-1.5 shrink-0">
                            <button type="button" wire:click="addReadingStatus({{ $b->id }}, 'reading')" class="btn btn-ghost btn-xs font-black uppercase text-primary hover:bg-primary/5 rounded-lg px-2" tooltip="Currently Reading">📖 Read</button>
                            <button type="button" wire:click="addReadingStatus({{ $b->id }}, 'want_to_read')" class="btn btn-ghost btn-xs font-black uppercase text-amber-500 hover:bg-amber-500/5 rounded-lg px-2" tooltip="Wishlist">📌 Wish</button>
                            <button type="button" wire:click="addReadingStatus({{ $b->id }}, 'completed')" class="btn btn-ghost btn-xs font-black uppercase text-emerald-500 hover:bg-emerald-500/5 rounded-lg px-2" tooltip="Completed">✅ Done</button>
                        </div>
                    </div>
                @empty
                    <div class="p-4 text-center text-xs text-slate-400 font-bold uppercase tracking-wider">
        ...
        <x-slot:actions>
            <button type="button" wire:click="$set('addReadingModal', false)" class="btn btn-ghost btn-sm rounded-xl font-bold">Close</button>
        </x-slot:actions>
    </x-modal>

    {{-- ==================== MODAL: UPDATE READING PROGRESS / REVIEW ==================== --}}
    <x-modal wire:model="progressModal" title="Update Reading Progress" class="backdrop-blur-sm" box-class="rounded-3xl border border-slate-200/60 dark:border-slate-800/80 bg-white dark:bg-slate-900 shadow-2xl p-6">
        <div class="space-y-4">
                ...
                @if($activeInteraction)
                    <div class="bg-slate-50/50 dark:bg-slate-950/20 p-4 rounded-2xl border border-slate-200/50 dark:border-slate-800/80 flex gap-3">
                        ...
                        <div>
                            ...
                            <span class="text-xs text-slate-500">Total pages: <span class="font-semibold">{{ $activeInteraction->book->pages_count ?? 'Unspecified' }}</span></span>
                        </div>
                    </div>
                @endif
            @endif
            ...
            @if($readingSubTab === 'reading')
                <div>
                    <label class="block text-xs font-bold text-slate-450 dark:text-slate-400 mb-1.5">Pages Read</label>
            ...
            {{-- Rating stars --}}
            <div>
                <label class="block text-xs font-bold text-slate-450 dark:text-slate-400 mb-2">Your Rating</label>
                <div class="flex items-center gap-2">
                    @for($i = 1; $i <= 5; $i++)
                    ...
                    @endfor
                    @if($progressRating)
                        <button type="button" wire:click="$set('progressRating', null)" class="text-xs text-slate-400 hover:text-rose-500 font-bold ml-2">Clear</button>
                    @endif
                </div>
            </div>
            ...
            {{-- Review --}}
            <div>
                <label class="block text-xs font-bold text-slate-450 dark:text-slate-400 mb-1.5">Your Review / Personal Note</label>
                <textarea wire:model="progressReview" rows="4" placeholder="Write down your thoughts, takeaways, or short review..."
        ...
        <x-slot:actions>
            <button type="button" wire:click="$set('progressModal', false)" class="btn btn-ghost btn-sm rounded-xl font-bold">Cancel</button>
