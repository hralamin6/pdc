<div
    x-data="{
        ...
        init() {
            if (this.hasTimer) {
                setInterval(() => { if (this.remaining > 0) this.remaining--; else $wire.timerTick(); }, 1000);
            }
            if (typeof Echo !== 'undefined') {
                Echo.channel('quiz.{{ $quiz->id }}')
                    .listen('.QuizLiveStarted', (e) => { $wire.dispatch('quiz-live-started', { quiz_id: e.quiz_id }); })
                    .listen('.LeaderboardUpdated', (e) => { $wire.dispatch('leaderboard-updated', { quiz_id: e.quiz_id, leaderboard: e.leaderboard }); });
            }
        }
    }">
...
<div class="max-w-2xl mx-auto py-12 text-center space-y-8">
    ...
    <div>
        ...
        <p class="text-base-content/60">{{ __('Waiting for the host to start the quiz…') }}</p>
    </div>
    <div class="grid grid-cols-3 gap-4 max-w-sm mx-auto">
        <div class="bg-base-100 rounded-2xl p-4 text-center border border-base-content/5">
            ...
            <div class="text-xs text-base-content/50 mt-1">{{ __('Questions') }}</div>
        </div>
        <div class="bg-base-100 rounded-2xl p-4 text-center border border-base-content/5">
            ...
            @endif
            <div class="text-xs text-base-content/50 mt-1">{{ __('Time Limit') }}</div>
        </div>
        <div class="bg-base-100 rounded-2xl p-4 text-center border border-base-content/5">
            <div class="text-2xl font-black text-accent">{{ $quiz->total_marks }}</div>
            <div class="text-xs text-base-content/50 mt-1">{{ __('Total Marks') }}</div>
        </div>
    </div>
    ...
    <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-error/10 text-error text-sm font-medium">
        <x-icon name="o-minus-circle" class="w-4 h-4" /> {{ __('Negative marking is enabled') }} (–{{ $quiz->negative_mark_value * 100 }}%)
    </div>
    @endif
    @if($quiz->available_from && $quiz->available_from > now())
        <div class="inline-flex flex-col items-center justify-center p-4 bg-warning/10 border border-warning/20 rounded-2xl"
             x-data="{ 
            ...
            <span class="text-xs font-black uppercase tracking-widest text-warning mb-1">{{ __('Starting In') }}</span>
            <span class="font-mono text-3xl font-black text-warning" x-text="format()"></span>
        </div>
    @else
        <div class="flex items-center justify-center gap-2 text-sm text-base-content/40">
            <span class="loading loading-dots loading-sm"></span> {{ __('Waiting for host to start') }}
        </div>
    @endif
</div>
...
<div class="max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-4">
        ...
        <div class="bg-base-100 rounded-2xl border border-base-content/5 shadow-sm">
            <div class="px-5 py-3 border-b border-base-content/5 flex items-center gap-2">
                ...
                <span class="text-xs text-base-content/40">{{ $q->marks }} {{ __('marks') }}</span>
                @if($quiz->negative_marking && $q->type !== 'short_text')
                    <span class="text-xs text-error ml-auto">-{{ $q->marks * $quiz->negative_mark_value }} {{ __('wrong') }}</span>
                @endif
            </div>
        ...
        <div class="flex justify-between">
            <x-button icon="o-arrow-left" label="{{ __('Prev') }}" class="btn-ghost" wire:click="prevQuestion" :disabled="$currentIndex === 0" />
            @if($currentIndex < $totalQuestions - 1)
                <x-button icon-right="o-arrow-right" label="{{ __('Next') }}" class="btn-primary" wire:click="nextQuestion" />
            @else
                <x-button icon="o-check-circle" label="{{ __('Submit') }}" class="btn-success shadow-lg shadow-success/30" wire:click="confirmSubmit" />
            @endif
        </div>
        @endif
    </div>
    ...
    <div class="bg-base-100 rounded-2xl border border-base-content/5 p-4">
        <h3 class="font-bold text-sm mb-3 flex items-center gap-2">
            <x-icon name="o-trophy" class="w-4 h-4 text-warning" /> {{ __('Live Leaderboard') }}
        </h3>
        @if(empty($leaderboard))
            <div class="text-center py-6 text-base-content/30 text-xs">{{ __('Be the first to submit!') }}</div>
        @else
            <div class="space-y-2">
                ...
                <div class="flex items-center gap-2 p-2 rounded-xl {{ ($entry['is_me'] ?? false) ? 'bg-primary/10 border border-primary/20' : '' }}">
                    ...
                    <span class="flex-grow text-xs font-medium truncate {{ ($entry['is_me'] ?? false) ? 'text-primary' : '' }}">
                        {{ $entry['name'] }} {{ ($entry['is_me'] ?? false) ? __('(you)') : '' }}
                    </span>
        ...
        <div class="mt-4 border-t border-base-content/5 pt-4">
            <p class="text-xs font-bold text-base-content/40 mb-2">{{ __('Questions') }}</p>
...
<x-modal wire:model="showSubmitConfirm" title="{{ __('Submit Quiz?') }}" box-class="max-w-sm">
    <div class="text-center space-y-3 py-2">
        ...
        <p class="font-bold">{{ $answeredCount }}/{{ $totalQuestions }} {{ __('answered') }}</p>
        @if($totalQuestions - $answeredCount > 0)
            <p class="text-warning text-sm">{{ $totalQuestions - $answeredCount }} {{ __('unanswered will score 0.') }}</p>
        @endif
    </div>
    <x-slot:actions>
        <x-button label="{{ __('Keep Going') }}" wire:click="$set('showSubmitConfirm', false)" class="btn-ghost" />
        <x-button label="{{ __('Submit Now') }}" icon="o-check" wire:click="submitQuiz" class="btn-success" spinner="submitQuiz" />
    </x-slot:actions>
</x-modal>
...
<div class="max-w-2xl mx-auto space-y-6 py-4">
    <div class="bg-base-100 rounded-3xl border border-base-content/5 shadow-xl p-8 text-center">
        <h1 class="text-2xl font-black mb-2">{{ __('Quiz Complete!') }}</h1>
        ...
        @if($attempt?->points_awarded > 0)
            <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 text-primary font-bold text-sm mb-4">
                <x-icon name="o-star" class="w-4 h-4" /> +{{ $attempt->points_awarded }} {{ __('points earned!') }}
            </div>
        @endif
    </div>
    ...
    <div class="bg-base-100 rounded-2xl border border-base-content/5 p-5">
        <h3 class="font-bold mb-4 flex items-center gap-2">
            <x-icon name="o-trophy" class="w-5 h-5 text-warning" /> {{ __('Final Leaderboard') }}
        </h3>
        <div class="space-y-2">
            @foreach($leaderboard as $entry)
            <div class="flex items-center gap-3 p-3 rounded-xl
                ...
                <div class="flex-grow">
                    ...
                    @if($entry['points_awarded'] > 0)
                        <p class="text-xs text-base-content/40">+{{ $entry['points_awarded'] }} {{ __('pts') }}</p>
                    @endif
                </div>
